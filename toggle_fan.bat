@echo off
REM FengFanControl — 一键切换 自定义静音风扇 / 出厂EC控制
...
title FengFanControl Toggle

echo FengFanControl — Toggle
echo ======================
echo.
...
if %ERRORLEVEL% EQU 0 (
    echo [*] FengFanControl is RUNNING. Switching to FACTORY mode...
    "%PYTHON%" "%RESTORE%"
    if %ERRORLEVEL% EQU 0 (
        echo.
...
echo [*] FengFanControl is STOPPED. Switching to QUIET mode...
start /B "" "%PYTHON%" -u "%SCRIPT%" --quiet
if %ERRORLEVEL% EQU 0 (
    echo [OK] FengFanControl started (quiet mode).
) else (
